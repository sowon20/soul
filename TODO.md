...
### 해결: ChromaDB + 로컬 임베딩
- [x] ChromaDB 설치 (`pip install chromadb`)
- [x] ChromaDB 서버 실행 (`chroma run --path ./data/chroma --port 8000`)
- [x] Node.js 클라이언트 (`npm install chromadb`)
- [x] 로컬 임베딩 모델 (`@xenova/transformers` + `all-MiniLM-L6-v2`)
- [x] `vector-store.js` 유틸 생성
- [x] 기존 메시지 112개 마이그레이션 완료
- [x] `builtin-tools.js` recall_memory → 벡터 검색으로 교체
- [x] `memory-layers.js` 새 메시지 저장 시 벡터 자동 저장

### 테스트 결과
- "처음 만남" 검색 → "처음 만나서 반가워요" 찾음 (similarity 0.414)
- "첫 대화" 도 찾음 (similarity 0.073)
- 의미적 유사도 검색 작동 확인!

### 주의사항
- ChromaDB 서버 필요: `nohup chroma run --path ./data/chroma --port 8000 &`
- 서버 재시작 시 ChromaDB도 띄워야 함
- TODO: systemd 서비스로 등록하거나 pm2에 추가
...
## 🧹 리팩토링 필요 (Phase 4 이후)
- [x] 전체 코드 하드코딩 점검 (경로, URL, 설정값) ✅ 2026-01-27 완료
- [x] 중복 코드 유틸로 분리 (api-keys.js, backfill-tags.js 통합) ✅ 2026-01-27 완료
- [x] 개인정보 제거 (sowon → default) ✅ 2026-01-27 완료
- [ ] 안 쓰는 코드/파일 삭제
- [ ] 패턴 일관성 통일 (에러 처리, 응답 형식 등)
- [x] conversation-store.js 외 다른 파일도 설정 연동 점검 ✅

## 🧠 자기학습 규칙 시스템 (Phase 4 핵심) ⭐⭐⭐
Soul이 대화하면서 스스로 규칙을 만들고 학습하는 시스템

### 개념
- 대화 중 깨달은 것 → 자기 규칙으로 저장
- "소원이는 이거 모름" → "알아서 다 찾아보기"
- "이렇게 했더니 혼남" → "다음엔 안 함"
- 매 대화마다 관련 규칙 시스템 프롬프트에 자동 주입

### 토큰 관리
- [ ] 규칙 압축 - 비슷한 건 로컬 LLM이 병합
- [ ] 중요도 관리 - 자주 쓰이면 활성, 안 쓰이면 아카이브
- [ ] 카테고리별 로드 - 상황에 맞는 규칙만 (시스템/코딩/일상)
- [ ] 토큰 상한선 - 규칙 최대 500토큰, 넘으면 자동 압축

### 구현
- [ ] self_rules 컬렉션 (MongoDB)
- [ ] 규칙 추가/수정/삭제 API
- [ ] 대화 후 로컬 LLM이 규칙 추출
- [ ] 시스템 프롬프트 빌더에서 규칙 주입
- [ ] 규칙 압축/병합 로직
...
### ✅ 오늘 완료 (저녁) - 하드코딩 제거 및 개인정보 정리 ⭐⭐⭐
> ⚠️ **주의**: 다른 Claude Code 세션이 파일 삭제해서 6시간 작업 손실됨. 대화 로그에서 복구 진행.

- **하드코딩 제거**
  - `vite.config.js`: `VITE_API_URL` 환경변수 지원
  - `socket-client.js`: Vite dev(5173) 포트 지원
  - `Profile.js`: userId 기본값 'sowon' → 'default'
  - `update-role-models.js`: dotenv 설정 추가

- **개인정보 제거**
  - 소원/sowon → User/default 변경
  - `mcp/server-config.json`: 개인 MCP URL 제거, 빈 설정으로 초기화
  - `server-config.example.json` 예제 파일 추가

- **스크립트 통합 (중복 제거)**
  - `api-keys.js` 통합 (setup/update/migrate/status 기능)
    - 삭제: setup-api-keys.js, update-api-keys.js, migrate-api-keys.js
  - `backfill-tags.js` 통합 (standalone/alba-worker 모드)
    - 삭제: backfill-tags-standalone.js

- **WebSocket 연결 수정**
  - socket-client.js: Vite dev(5173), 빌드(3080) → 백엔드(3001) 연결
  - 대시보드 WebSocket 인디케이터 연동

- **기타**
  - `.env.example`: AI API 키 설정 섹션 추가
  - `dashboard-manager.js`: 서버 상태 및 AI 사용 통계 완전 복구 (파일 손상됨)

- **커밋**: `7628fed` (43파일, +2872/-937)

### ✅ 오늘 완료 (새벽 3시)
- **SSH Commander MCP 연동**
  - /tools, /logs API 추가
  - 웹소켓 실시간 연동 (AI 명령 [AI] 배지 표시)
  - 터미널 열면 이전 로그 불러오기
- **도구 사용 UI 기본 구현**
  - tool_use 태그로 응답에 포함
  - 🔧 도구 사용 토글 버튼
- **메모리 경로 설정 수정**
  - conversation-store.js가 DB 설정 경로 사용하도록 수정

### 🔲 다음 할 것 - 도구 사용 UI 개선 ⭐⭐
- **MCP 서버 이름 표시** (mcp_123... → "터미널")
  - 서버 ID → DB에서 이름 매핑
  - `터미널 > execute` 형식
- **실행 중 실시간 표시**
  - "🔧 터미널 실행 중..." + 스피너
...
### ✅ 오늘 완료 (새벽) - MCP 서버 편집 + 독 연동 ⭐
- **MCP 서버 편집 모달 확장**
  - 아이콘 선택 기능 (9종 아이콘)
  - UI URL 설정 (독 클릭 시 열리는 페이지)
  - "독에 표시" 체크박스
  - 저장 시 자동으로 독 업데이트
- **독 설정 DB 연동**
  - GET/PUT /api/config/dock API
  - showInDock 체크된 MCP 서버 자동 추가
  - 고정 아이템: 터미널, 마이크, 설정
- **Hub Server 도구 개수 수정**
  - context-tool.js 실제 도구 4개 파싱

### 🔲 다음 할 것 - 독(Dock) 편집 모드
- **기본 독 아이템** → 설정만
- **롱프레스 편집 모드**
  - 설정 아이콘 제외하고 순서 변경 가능
  - 아이콘 흔들림 + X버튼
  - 드래그앤드롭 순서 변경
- **아이콘 시스템 확장**
  - 사용자 이미지 업로드 또는
...
### ✅ 어제 완료 (1/26 밤 23시) - 캔버스 탭 시스템 ⭐
- **캔버스 패널 탭 시스템 구현**
  - 크롬 탭처럼 여러 MCP UI 동시 열기
  - 탭 전환, 닫기 기능
  - 독 클릭 → 캔버스 탭으로 열림
- **Todo UI 스타일 정리**
  - 스크롤바 숨김, 여백 최적화
  - Memo 탭: 프로그레스 바 숨김, textarea 100%

### 🔲 다음 할 것 - 독(Dock) 편집 모드
- **기본 독 아이템** → 설정만 (MCP showInDock 체크된 것 + 설정)
- **롱프레스 편집 모드**
  - 설정 아이콘 제외하고 순서 변경 가능
  - 아이콘 흔들림 + X버튼
  - 드래그앤드롭 순서 변경
...
- **Todo UI 스타일 정리**
  ...
  - 섹션 추가/삭제 (커스텀 모달)
- **UI 개선**
  - 뱃지 오른쪽 끝 정렬 (flex spacer)
  - textarea 세로만 resize 가능
  - 커스텀 삭제 확인 모달 (브라우저 confirm 대체)
- **MCP 도구 description 상세화**
  - 포맷 예시 포함 (AI가 올바른 형식으로 저장하도록)
  - read_todo/write_todo/read_memo/write_memo 6개 도구
...
### 📌 MCP 연동 남은 작업
- [x] Todo, SSH, 바램펫 서버에도 `/api/tool-call` 엔드포인트 추가 ✅ (이미 구현됨 확인 01-26)
